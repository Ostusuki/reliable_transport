...
def receiver(receiver_ip, receiver_port, window_size):
    ...
    while True:
        ...
        if header.type == START_TYPE and not connection_active:
            if header.seq_num == 0:
                connection_active = True
                ack_pkt = create_ack(1)
                s.sendto(ack_pkt, addr)

        elif header.type == DATA_TYPE and connection_active:
            seq_num = header.seq_num
            if seq_num > 0:
                data_buffer[seq_num] = payload
                if seq_num > max_seq_received:
                    max_seq_received = seq_num
                ack_pkt = create_ack(seq_num)
                s.sendto(ack_pkt, addr)

        elif header.type == END_TYPE and connection_active:
...
def main():
    ...
    parser.add_argument("receiver_ip", help="IP of receiver")
    parser.add_argument("receiver_port", type=int, help="Port of receiver")
